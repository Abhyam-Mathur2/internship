...
export function Login() {
  const { login } = useAuth();
  const [form, setForm] = useState({ email: 'employee@payrollaudit.test', password: 'password' });
  ...
  return (
    <form onSubmit={submit} className="card w-full max-w-md justify-self-center">
      ...
      <p className="mt-1 text-sm text-slate-500">Use your employee account to audit payslips.</p>
      ...
      <p className="mt-4 text-center text-sm text-slate-500">
        New here? <Link className="font-semibold text-blue-600" to="/register">Create account</Link>
      </p>
    </form>
  );
}
